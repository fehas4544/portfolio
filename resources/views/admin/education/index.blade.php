...
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-3xl font-bold text-slate-800 tracking-tight">Academic History</h1>
        <p class="text-slate-500 font-medium">Manage your educational background and degrees</p>
    </div>
    <a href="{{ route('admin.education.create') }}" class="btn-gradient">
        Add New Record
    </a>
</div>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-slate-50/50 border-b border-slate-100">
                    <th class="px-8 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Institution & Degree</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($education as $item)
                    <tr class="hover:bg-slate-50/30 transition-colors">
                        <td class="px-8 py-4">
                            <div>
                                <h4 class="text-sm font-bold text-slate-800">{{ $item->degree }}</h4>
                                <p class="text-xs text-blue-600 font-bold uppercase tracking-tighter">{{ $item->institution }}</p>
                            </div>
                        </td>
                        ...
                        <td class="px-8 py-4 text-right">
                            <div class="flex items-center space-x-3">
                                <a href="{{ route('admin.education.edit', $item) }}" class="text-slate-400 hover:text-blue-600 transition-colors">
                ...
                @empty
                    <tr>
                        <td colspan="3" class="px-8 py-12 text-center text-slate-500 font-medium">No education records found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
